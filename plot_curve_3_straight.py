...
def read_estimation_robot(test_number, est_type):
	...
	phi_est = data[:,3]
	#print(phi_est)
	#plt.plot(t_est/1e9, phi_est)
	#plt.savefig("./" + est_type + "_data" + test_number + ".png")
...
def write_to_file(test_number, est_type, mean, stdev):
	f = open("./evaluation2_results.txt", "a")
...
###
	phi_plot_est = np.zeros((0,1))
	t_plot_est = np.zeros((0,1))

	for k in range(len(t_est)):
		if t_est[k] > t_true[0] and t_est[k] < t_true[len(t_true)-1]:
			phi_plot_est = np.append(phi_plot_est, phi_est[k])
			t_plot_est = np.append(t_plot_est, t_est[k])



	plt.figure(est_type)
	plt.plot(t_plot_est - t_plot_est[0], phi_plot_est,'--')
	plt.plot(t_plot_est - t_plot_est[0], phi_plot_est,'o')
	#plt.title()
	plt.ylabel(est_type +" phi-estimation [rad]")
	plt.xlabel("time [s]")
	plt.savefig("./testplot" + est_type)

	if est_type == 'SF':
		plt.figure("local")
		plt.plot(t_true -t_true[0], phi_true,'--')
		plt.plot(t_true -t_true[0], phi_true,'o')
		#plt.title()
		plt.ylabel("measured phi-estimation-error to localization [rad]")
		plt.xlabel("time [s]")
		plt.savefig("./error_local")
